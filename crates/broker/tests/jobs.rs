...
#[tokio::test]
async fn submit_http_probe_to_loopback_fails_closed() {
    let (app, _, admin) = app_with_admin(8).await;
    let body = json!({
        "name": "http_probe",
...
#[tokio::test]
async fn submit_http_fetch_to_loopback_fails_closed() {
    let (app, _, admin) = app_with_admin(8).await;
    let body = json!({
        "name": "http_fetch",
