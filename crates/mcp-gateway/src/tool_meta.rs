...
pub(crate) fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "workflow_start" => "Create and bind a session, page, and workflow, optionally navigating to url. Requires session:read, session:write, page:write. On failure, inspect session_list.",
        "workflow_observe" => "Observe retained or live accessibility evidence. Requires browser:mutate; forms need page:read. Defaults evidenceDetail=compact.",
        ...
        "click" => "Click a selector or resolved target; pass workflowHandle, optional Shift/Ctrl/Alt/Meta modifiers. Requires browser:mutate. On failure with targetNotFound or targetAmbiguous, refresh a11y_snapshot.",
        ...
        "intent_complete_form" => "Fill ordered named fields in one verified intent; never submits. Pass workflowHandle. Requires browser:mutate and intent:execute. Fields resolve just-in-time; revealedBy activates first. Defaults evidenceDetail=compact. On failure retry remaining fields.",
