...
pub(crate) fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "job_submit" => "Submit a named job. Built-in handlers: echo (returns payload), sleep (payload.ms, default 1000, cap 30000), http_probe (payload.url, method HEAD|GET, timeoutMs), http_wait (payload.url, timeoutMs/intervalMs/probeTimeoutMs). Requires job:submit. Same as POST /v1/jobs. On failure with notFound, retry submit.",
