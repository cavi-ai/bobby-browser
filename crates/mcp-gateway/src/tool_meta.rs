...
pub(crate) fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "intent_follow" => "Activate a described control and verify its resulting state. Pass expectedState; expectedDestination remains accepted. Boundary when boundary is true, else Reconciliable. Requires browser:mutate and intent:execute. With boundary, autoCheckpoint defaults true. On failure with needsReconciliation, do not retry; call recovery_status. On targetNotFound, snapshot again.",
