...
pub(crate) fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "click_and_wait_for_popup" => "Click a control and wait for a window.open popup page to register so page_list can drive it (Boundary). Requires browser:mutate. autoCheckpoint defaults to true and mints the checkpoint in this call; pass false and pin commandId/attemptId with checkpoint_save first to author invariants. Prefer this over plain click when the control opens a popup. On failure with deadlineExceeded, confirm the click actually opens a window; on needsReconciliation, do not retry -- call recovery_status.",
        ...
        "control_action" => "Apply one native control action and reread state. Requires browser:mutate and file:upload for setFiles. selectOne and selectMany accept an option's value or its visible label. On failure with targetNotFound, refresh form_snapshot.",
        ...
        "download_url" => "Download a URL into the session's downloads, bounded by a byte limit. Requires browser:mutate and file:download. Produces a download artifact with its size and checksum. On failure with networkPolicyDenied: loopback and private destinations are denied unless the operator sets http.allow_loopback / http.allow_private_network; for a file the page already links, click the link instead.",
        ...
        "job_submit" => "Submit a named job. Built-in handlers: echo, sleep (ms, default 1000, cap 30000), http_probe (url, HEAD|GET, timeoutMs), http_wait (url, timeoutMs/intervalMs/probeTimeoutMs; contains polls via http_fetch), http_fetch (url, GET, truncated body, contains/maxBodyBytes/timeoutMs). All read payload. Requires job:submit. Same as POST /v1/jobs. On failure with notFound, retry submit.",
