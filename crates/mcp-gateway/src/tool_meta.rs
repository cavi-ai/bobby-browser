...
pub(crate) fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "upload_files" => "Set files on a file input. Accepts selector, target, or form_snapshot controlId. Pass expectedState for one retained Boundary command; autoCheckpoint defaults true. Requires browser:mutate and file:upload. Returns upload and confirmation evidence. On failure with needsReconciliation, call recovery_status; on policyDenied, use a configured upload root.",
        ...
        "intent_complete_form" => "Fill ordered named fields in one verified intent; never submits. Pass workflowHandle. Requires browser:mutate and intent:execute. Hints per field; top-level hints applies only for a single field. Prefer over repeated intent_fill calls. Fields resolve just-in-time: conditional fields resolve even when absent. Success defaults evidenceDetail=compact. On failure, retry remaining fields.",
        "intent_submit_and_verify" => "Submit once and verify post-state. Pass workflowHandle; ids if it dies. Requires browser:mutate and intent:execute. networkQuiet returns submitSettlement=settled|validationRejected; rejection includes formValidation issues, so do not inspect or blindly resubmit. On failure with needsReconciliation call recovery_status.",
        ...
        "intent_follow" => "Activate and verify a described control. Prefer over click plus wait_for. Pass expectedState. Requires browser:mutate and intent:execute. Defaults evidenceDetail=compact; use full for diagnostics. boundary=true is Boundary; autoCheckpoint defaults true. On failure with needsReconciliation, do not retry; call recovery_status. On targetNotFound, snapshot again.",
        ...
        "job_submit" => "Submit a built-in job: echo; sleep (ms); http_probe (url); http_wait (url, contains); http_fetch (url). Requires job:submit. Same as POST /v1/jobs. On failure after submission, use job_status on the returned id; unknown names fail asynchronously, so do not resubmit.",
