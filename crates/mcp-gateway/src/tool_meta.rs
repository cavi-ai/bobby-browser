...
pub(crate) fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "a11y_snapshot" => "Capture a compact accessibility tree for a page, capped at 2048 nodes, with command-ready targets on actionable nodes. Descends one level into same-process iframes; in-frame targets carry their frame hop. Links carry their url. Requires browser:mutate. Start here: pass a node's target into an intent_* tool rather than guessing a selector.",
