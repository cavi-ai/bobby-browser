...
pub(crate) fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "click_and_wait_for_popup" => "Click a control and wait for a window.open popup page to register so page_list can drive it. Requires browser:mutate. Prefer this over plain click when the control opens a popup. On failure with deadlineExceeded, confirm the click actually opens a window.",
