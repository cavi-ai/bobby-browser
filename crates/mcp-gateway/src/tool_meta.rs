...
pub(crate) fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "intent_complete_form" => "Fill ordered named fields in one verified intent; never submits. Pass workflowHandle. Requires browser:mutate and intent:execute. A field name may be a form_snapshot controlId. Prefer over repeated intent_fill calls. Fields resolve just-in-time: conditional fields may follow their revealer even if absent. Success defaults evidenceDetail=compact. On failure, retry only remaining fields.",
