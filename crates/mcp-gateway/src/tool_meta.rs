...
pub(crate) fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "intent_follow" => "Activate a described control and verify the result. Prefer over click plus wait_for or observation. Pass expectedState; expectedDestination is accepted. Boundary when boundary=true, else Reconciliable. Requires browser:mutate and intent:execute. Boundary autoCheckpoint defaults true. On failure with needsReconciliation, do not retry; call recovery_status. On targetNotFound, snapshot again.",
