...
fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "workflow_recover" => "Recover a workflow from its last verified checkpoint, resuming, restarting, or flagging reconciliation. Requires recovery:write. Produces recovery evidence and the decision reached. On failure with notFound, no checkpoint exists yet -- call checkpoint_save before recovering.",
        ...
        "navigate" => "Navigate a page to a URL and wait for the requested load state. Requires browser:mutate. Produces navigation evidence with the settled URL and title. On failure with deadlineExceeded, the page did not reach that state in time -- retry with a longer timeout_ms or a lighter wait_until.",
        ...
        "control_action" => "Perform one typed native form-control action and return the reread control state. Requires browser:mutate. Produces control-action evidence with the post-action value. On failure with targetNotFound, take a fresh form_snapshot and pass the new target.",
        ...
        "cookie_set" => "Store cookies on a page's jar. Requires browser:mutate. Produces the updated cookie-jar state. On failure with invalidRequest, a cookie's domain, secure flag, or SameSite setting doesn't match the page -- scope the domain to the page's own host.",
