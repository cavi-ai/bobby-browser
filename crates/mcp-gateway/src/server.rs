...
fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "network_log" => "Dump the page's recorded network log as a HAR artifact, then clear the buffer unless clear is false. Requires browser:mutate. Produces HAR-artifact evidence with entry count, byte size, and checksum. On failure: verificationFailed means no HAR was captured; browserCommandFailed means the engine could not persist the HAR; internal means HAR serialization or artifact write failed -- none are caller-fixable; retry, and report if it persists.",
        _ => "Runtime operation.",
    }
}
