...
- MCP `job_submit` / `job_status` / `job_cancel` mirror HTTP `/v1/jobs` (same caps). Advertised in `full`, `act`, and `verify` when a job port is attached (`bobby mcp-stdio` and `bobby serve` MCP HTTP). Built-in handlers: `echo`, `sleep`, `http_probe`, and `http_wait` (SSRF-safe HEAD/GET under the download destination policy; `http_wait` polls until success); `bobby://job-handlers` documents payloads; `bobby doctor` reports them under `job-handlers`.
