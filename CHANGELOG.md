...
- MCP `job_submit` / `job_status` / `job_cancel` mirror HTTP `/v1/jobs` (same caps). Advertised in the `verify` phase; callable whenever a job port is attached (`bobby mcp-stdio` and `bobby serve` MCP HTTP). Built-in handler names are `echo` and `sleep`; `bobby doctor` reports them under `job-handlers`, and the `job_submit` tool description names the payloads.
...
- `bobby init --preset agent` mints a bootstrap without `authority:admin`; heal respects the preset marker and never widens an agent floor to unrestricted. Default `bobby init` remains unrestricted. `bobby doctor` reports `bootstrap-preset`.
