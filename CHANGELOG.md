...
- MCP `job_submit` / `job_status` / `job_cancel` mirror HTTP `/v1/jobs` (same caps). Advertised in `full`, `act`, and `verify` when a job port is attached (`bobby mcp-stdio` and `bobby serve` MCP HTTP). Built-in handlers remain `echo` and `sleep`; `bobby://job-handlers` documents payloads; `bobby doctor` reports them under `job-handlers`.
