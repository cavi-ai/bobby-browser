...
- `[mcp] startup_toolset`, overridden by `BOBBY_MCP_TOOLSET`: `full` (default), `explore`, `act`, `intent`, `verify`. Sets the phase at connect. `tools/list` is 128,029 bytes on `full`, 42,253 on `explore`.
...
- `intent_submit_and_verify` and `intent_follow` accept `autoCheckpoint`. A Boundary command took three calls — pin `commandId`/`attemptId`, `checkpoint_save` naming those exact ids, then submit — because a `WorkflowCheckpoint` needs `restartUrl` and `currentUrl` and nothing on the runtime interface exposes live page state. The runtime now mints it and returns its `checkpointId`. `Executor::validate` is unchanged and still matches on all five fields; a checkpoint that fails to save fails the submit. The manual path is unchanged and is still the way to author `invariants` or `replayableInputs`.
