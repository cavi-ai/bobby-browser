...
- `fingerprint_conformance` resolves Chrome from `BOBBY_CHROME_EXECUTABLE` before `CHROME_PATH`. It read only `CHROME_PATH`, which CI does not set, so it launched nothing. `chromium_collector_dogfood_passes` is skipped in CI pending issue #125: its `chromeRuntime` probe fails on stock Chrome, where `window.chrome.runtime` is undefined on ordinary pages.
