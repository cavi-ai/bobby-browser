...
function isFormControl(value: unknown): value is FormControl {
  ...
  // fields (null placeholder, empty options, constraints with nothing to
  // say, a label identical to accessibleName) are omitted on the wire.
  if (!hasExactKeys(value, ["id", "controlKind", "state", "validity", "supportedOperations"], ["formId", "groupId", "target", "accessibleName", "label", "description", "placeholder", "autocomplete", "constraints", "options"]) || !boundedText(value.id, 128) || !(value.formId === undefined || value.formId === null || boundedText(value.formId, 128)) || !(value.groupId === undefined || value.groupId === null || boundedText(value.groupId, 128)) || !(value.target === undefined || value.target === null || isFormControlTarget(value.target)) || !oneOf(value.controlKind, CONTROL_KINDS) || !nullableBoundedText(value.accessibleName ?? null, 2048) || !nullableBoundedText(value.label ?? null, 2048) || !nullableBoundedText(value.description ?? null, 2048) || !nullableBoundedText(value.placeholder ?? null, 2048) || !nullableBoundedText(value.autocomplete ?? null, 2048)) return false;
  const state = value.state;
...
function isFormDescriptor(value: unknown, globalIds: Set<string>): value is FormDescriptor {
  ...
  for (const control of value.controls) { if ((control.formId ?? null) !== value.id || controls.has(control.id) || globalIds.has(control.id)) return false; controls.set(control.id, control); globalIds.add(control.id); }
  ...
  if ([...controls.values()].some((control) => (control.groupId ?? null) !== null && (!groups.has(control.groupId as string) || !formGroups.some((group) => isRecord(group) && group.id === control.groupId && Array.isArray(group.controlIds) && group.controlIds.includes(control.id))))) return false;
