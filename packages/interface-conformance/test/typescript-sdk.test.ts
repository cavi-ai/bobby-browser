...
test("TypeScript SDK executes every canonical step on the authenticated Chrome runtime", { timeout: 1_800_000 }, async (t) => {
  ...
  let client: BrowserRuntimeClient | undefined = new BrowserRuntimeClient({ baseUrl: boot.endpoint, bearerToken: boot.token });
  let denied: BrowserRuntimeClient | undefined = new BrowserRuntimeClient({ baseUrl: boot.endpoint, bearerToken: boot.deniedToken });
  ...
  if (samples !== undefined) {
    await runPersistentPerformance({
      ...
      samples,
      run: execute,
      disconnect: async () => { client = undefined; denied = undefined; },
    });
  } else {
