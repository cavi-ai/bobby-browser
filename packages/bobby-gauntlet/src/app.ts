...
export function mountNorthstar(root: HTMLElement, api: NorthstarApi): NorthstarApp {
  ...
  const render = async (route: Route): Promise<void> => {
    const content = await northstarPage(document, route, api, router);
    root.replaceChildren(applicationShell(document, content, router));
  };
...
if (typeof document !== "undefined") {
  ...
  if (window.location.pathname.startsWith("/station/") || window.location.pathname === "/championship") {
    mountGauntlet(root, window.location.pathname, window.location.search);
  } else {
    ...
    void mountNorthstar(root, new NorthstarApi(runId)).navigate(window.location.pathname);
  }
}
