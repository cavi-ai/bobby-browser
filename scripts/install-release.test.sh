...
printf '%s\n' '#!/usr/bin/env bash' 'if [[ "${1:-}" == "--version" ]]; then echo "bobby-browser 0.16.0"; exit 0; fi' 'if [[ "${1:-}" == "profiles" && "${2:-}" == "--json" ]]; then echo '\''[{"name":"desktop"},{"name":"headless-ci"},{"name":"openshell"},{"name":"remote"}]'\''; exit 0; fi' 'exit 2' > "$stage/bobby"
