...
impl Target {
    ...
    fn is_page_or_iframe(&self) -> bool {
        self.is_page() || self.info.r#type == "iframe"
    }
    ...
    pub(crate) fn poll(&mut self, cx: &mut Context<'_>, now: Instant) -> Option<TargetEvent> {
        if !self.is_page_or_iframe() {
            // Page-like targets include out-of-process iframes. They need an
            // attached session for cross-origin frame automation, but remain
            // excluded from Browser::pages().
            return None;
        }
